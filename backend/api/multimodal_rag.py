...
def transcribe_audio_files(files_with_names):
    documents = []
    ...
    for file, file_name in files_with_names:
        ...
        document = Document(
            ...
            metadata={"format": "audio", "source": file_name}  # Adjust source if needed
        )
...
def create_documents_from_captions(captions):
    documents = []
    ...
    for image_filename, caption in captions:
        ...
        document = Document(
            ...
            metadata={"format": "image", "source": source_file, "page": int(page_number), "img": int(img_number)}
        )
        documents.append(document)
...
def create_documents_from_frames(captions):
    documents = []
...
def process_videos_in_directory(input_dir, output_dir_base, frame_rate):
    documents=[]
    ...
    for video_filename in os.listdir(input_dir):
        if video_filename.endswith((".mp4", ".avi", ".mov", ".mkv")):  # Add other video formats if needed
            ...
            document = create_documents_from_frames(transcriptions)
            for doc in document:
